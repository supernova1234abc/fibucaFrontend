...
*/}
  ...
  // Shifted padding-top to make room for the banner
  const cardStyle =
    ...
    <div className="flex flex-col items-center space-y-4 p-4">
      ...
      >
        ...
        <div className={cardStyle} style={{ fontFamily: 'Inter, sans-serif' }}>
          {/* Banner: full-width, curved top corners */}
          ...
          {/* Photo + Name + Role (shifted down by pt-8) */}
          <div className="absolute top-12 ml-3 flex flex-col items-center w-1/3 space-y-0 pt-1">
            ...
            <p className="text-xs  font-mono ml-2 text-black-700 text-center truncate space-y-0 max-w-[19rem]">
              {getFirstAndLastName(card?.fullName)}
            </p>


            {/* Role */}
            <p className="text-xs   text-gray-700 text-center space-y-0 truncate max-w-[10rem]">
              {card?.role || 'Position'}
            </p>
          </div>

          {/* QR Code */}
          <div className="absolute top-20 ml-24 left-36 pr-1  justify-center flex items-center">
            <QRCode
