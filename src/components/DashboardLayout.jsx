...
export default function DashboardLayout({ children, menus = [], user }) {
  ...
  const renderMenuButton = (menu, idx, compact = false) => {
    ...
    return (
      <button
        ...
        className={`no-force-dark group flex items-center justify-between w-full text-left rounded-xl transition-all duration-200 ${
      ...
      >
        ...
        {active && <FaChevronRight size={12} className="opacity-90" />}
      </button>
    );
  };
            ...
            <nav className="p-4 space-y-2">
              ...
              {sectionMenus.length > 0 && (
                <div className="pt-4 border-t border-slate-200">
                  ...
                  <div className="space-y-2">
                    {sectionMenus.map((menu, idx) => renderMenuButton(menu, idx, true))}
                  </div>
                </div>
              )}
            </nav>
